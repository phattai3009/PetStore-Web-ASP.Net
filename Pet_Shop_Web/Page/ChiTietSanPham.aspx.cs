...
public partial class Page_ChiTietThuCung : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            String ma = Request.QueryString["matc"].ToString();
            string conStr = "Data Source=PHATTAI\\PHATTAI; Initial Catalog =QLTC; User ID=sa; Password=123";
            using (SqlConnection con = new SqlConnection(conStr))
            {
                String cmdStr = "SELECT * FROM THUCUNG, GIONG WHERE THUCUNG.MAGIONG = GIONG.MAGIONG AND MATC =" + ma;
